...
const SpotTokens = ({ setIsOpen, searchTerm }) => {

    const dispatch  = useDispatch()

    const spotTokenList = useSelector(
        (state) => state?.perpetualsData?.spotTokenList
    );
    const filteredTokens = spotTokenList?.filter(token =>
                    ...
                    filteredTokens.map((item, ind) => {
                        return (
                            <div
                                ...
                                className={`group cursor-not-allowed relative   hover:bg-[#2a2a2a] px-4 py-1.5 border-b border-gray-800/50 transition-all duration-200`}
                            >
                                <div className="grid grid-cols-12 gap-4 items-center">
                                    ...
                                    <div className="col-span-3 flex items-center gap-2">
                                        <div className='flex items-center gap-2'>
                                            ...
                                            <div className='text-xs w-fit bg-[#11265B]/50 px-1 py-1 rounded-sm text-[#3b82f6]'>Spot</div>
                                        </div>
                                    </div>
                                    ...
                                    {/* 24h Change */}
                                    <div className="col-span-3 ">
                                        <div className={`font-medium text-[13px] flex items-center gap-1 ${Number(item?.change24h) > 0 ? "text-green-400" : "text-red-400"}`}>
                                            <div>{Number(item?.change24h)?.toFixed(2)}</div> /
                                            <div>{Number(item?.change24hPercent)?.toFixed(2)}%</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="col-span-3  ">
                                        <div className="text-white text-[13px] font-medium">
                                            {humanReadableFormatWithNoDollar(item?.volume, 2)}
                                        </div>
                                    </div>
                                </div>
                            </div>
