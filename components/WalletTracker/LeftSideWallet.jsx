...
function LeftSideWallet({ onChartOpen, walletData, setWalletData, setWalletChartData }) {
    ...
    const handleUpdate = async (wallet, index) => {
        ...
        await axios.put(`${BASE_URL}wavePro/users/walletTracking`, updatedWalletData).then((response) => {
            if (response.data?.success) {
                ...
                toast.success("Wallet name updated successfully!", { position: "top-center", style: { fontSize: "12px" } });

            } else {
                toast.error(response.data?.message || "Failed to update wallet name.");
            }
        }).catch((err) => {
    ...
    const handleTagUpdate = async (wallet, index) => {
        ...
        if (!updatedTag.trim() || JSON.stringify(newTags) === JSON.stringify(wallet.tag)) {
        ...
        try {
            const response = await axios.put(`${BASE_URL}wavePro/users/walletTracking`, updatedWalletData);
            ...
            if (response.data?.success) {
                ...
                toast.success("Tags updated successfully!", { position: "top-center", style: { fontSize: "12px" } });
            } else {
    ...
    const toggleNotification = async (wallet, index) => {
        ...
        try {
            const response = await axios.put(`${BASE_URL}wavePro/users/walletTracking`, {
                user: address,
                walletAddress: wallet.walletAddress,
                alert: updatedAlert,
            });
    ...
    return (
        <div className="bg-[#0A0A0B] text-white xl:h-[77vh] h-[40vh] overflow-y-scroll overflow-x-auto min-w-full">
            <div className="min-w-[500px]">
                {walletData.length === 0 ? (
                    <div className="text-center text-[#A8A8A8] py-5 h-[20vh] flex justify-center items-center">No Data Available</div>
                ) : (
                        ...
                        >
                            ...
                            <div className="flex-1 ml-3">
                                <div className="flex gap-2 items-center">
                                    ...
                                    <button onClick={() => handleCopy(wallet.walletAddress, index)} className="cursor-pointer">
                                        {copiedIndex === index ? (
                                ...
                                <div className="text-xs flex items-center gap-1">
                                    ...
                                    {editTagIndex === index ? (
                                        <input
                                            ...
                                            onKeyDown={(e) => e.key === "Enter" && handleTagUpdate(wallet, index)}
                                            autoFocus
                            ...
                            <div className="flex gap-6">
                                <Tooltip body={"Enable/Disable Alert"}>
                                    <div onClick={() => toggleNotification(wallet, index)} className="cursor-pointer">
                                        {wallet.alert ? (
                                ...
                                <RiDeleteBinLine onClick={() => handleDelete(wallet.walletAddress)} className="text-[#ED1B24] cursor-pointer hover:text-red-400" />
                            </div>
                        </div>
                    ))
                )}
