...
const CallerProfilePage  = ({params: dynamicId}) => {
    ...
    const [shownMainStatData, setShownUserData] = useState([]);
    ...
    const [yearlyUserData, setYearlyAlphaData] = useState([]);
    const [allTimeUserData, setAllTimeUserData] = useState([]);
    ...
    const [activeIntervalUserMainStat, setActiveIntervalUserMainStat] = useState('ALLTIME');
    ...
    const handleMainStatIntervalChange = (newInterval) => {
        setActiveIntervalUserMainStat(newInterval);
    };
    ...
    const timeIntervals = [
        { label: "All", value: "ALLTIME", data: allTimeUserData },
        { label: "1D", value: "DAILY", data: dailyUserData },
        { label: "7D", value: "WEEKLY", data: weeklyUserData },
        { label: "1M", value: "MONTHLY", data: monthlyUserData },
    ];
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            const apiUrl = `https://wave-scan-test-cebad401b8f2.herokuapp.com/getUserStats`;
            ...
            try {
                for (const interval of intervals) {
                    ...
                    if (interval === "ALLTIME") {
                        setShownUserData(response.data.userStat);
                        setAllTimeUserData(response.data.userStat);
                        setLoadingMainPage(false);
                    } else if (interval === "DAILY") {
                        setDailyUserData(response.data.userStat);
                    } else if (interval === "WEEKLY") {
                        setWeeklyUserData(response.data.userStat);
                    } else if (interval === "MONTHLY") {
                        setMonthlyUserData(response.data.userStat);
                    } 
                }
            } catch (error) {
        ...
        fetchUserData(); // Fetch in the background
    ...
    useEffect(() => {
        ...
        const bestAdminCall = shownMainStatData?.highestPerformanceCall
    ...
    useEffect(() => {
        const newShowTimeData = activeIntervalUserMainStat;
        if (newShowTimeData === "DAILY") {
        setShownUserData(dailyUserData);
        } else if (newShowTimeData === "WEEKLY") {
        setShownUserData(weeklyUserData);
        } else if (newShowTimeData === "MONTHLY") {
        setShownUserData(monthlyUserData);
        } else if (newShowTimeData === "YEARLY") {
        setShownUserData(yearlyUserData);
        } else if (newShowTimeData === "ALLTIME") {
        setShownUserData(allTimeUserData);
        }
    }, [activeIntervalUserMainStat]);
    ...
    useEffect(() => {
        ...
        if (newShowTimeData === "DAILY") {
            dataToShow = dailyUserData;
        } else if (newShowTimeData === "WEEKLY") {
            dataToShow = weeklyUserData;
        } else if (newShowTimeData === "MONTHLY") {
            dataToShow = monthlyUserData;
        } else if (newShowTimeData === "YEARLY") {
            dataToShow = yearlyUserData;
        } else if (newShowTimeData === "ALLTIME") {
            dataToShow = allTimeUserData;
        }
    
        if (!dataToShow) return; // ⛔ Don’t run if no data
    
        const sortedAdminCalls = dataToShow?.allCalls
        ? [...dataToShow.allCalls].sort((a, b) => b.performance - a.performance).slice(0, 20)
        : [];
    ...
    useEffect(() => {
        ...
        setGraphDataToShow(dataToShow);
         ...
         const newMaxGraphValue = getHighestPerformanceCount(dataToShow?.performanceCounts);
         setPerformanceOpacities(getPerformanceOpacities(dataToShow?.performanceCounts));
    ...
    const noCallsFoundMessage = (timeInterval) => {
        switch (timeInterval) {
            case 'ALLTIME':
                return 'No calls avaiable for User';
            case 'DAILY':
                ...
                <div className="flex justify-between py-6 items-center border-b border-[#404040]">
                    ...
                    <div className="xl:w-[67%] w-full xl:pl-6 px-6 flex justify-start items-center gap-8">
                        ...
                        <div className="flex flex-col shrink-0 py-0.5">
                            <div className="flex gap-2 onest w-full">
                                <p className="font-bold text-lg leading-6">{shownMainStatData?.userName || "Username"}</p>
                            ...
                            <div className="flex gap-2 onest text-[#A8A8A8] font-normal text-xs leading-4 items-center">
                                {/* {shownMainStatData?.adminName &&
                                    <>
                                        <button
                                            onClick={() => router.push(`/caller-profile/${shownMainStatData?.adminId}`)}
                        ...
                        <div className="flex items-center justify-center flex-col shrink-0">
                            <div className="flex gap-1 items-center">
                                ...
                                <Infotip body={`Rank based on performance in ${readableTimeInterval(activeIntervalUserMainStat)}`} />
                            </div>
                        ...
                        <div className="flex justify-between w-full">
                            <div className="flex flex-col items-center gap-1 py-4 onest">
                                <div className="flex gap-1 items-center">
                                    ...
                                    <Infotip body={`Total Calls made in ${readableTimeInterval(activeIntervalUserMainStat)}`} />
                                </div>
                                <div className="flex gap-[5px]">
                                    ...
                                    <p className="font-bold text-base leading-5">{shownMainStatData?.calls}</p>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col items-center gap-1 py-4 onest">
                                <div className="flex gap-1 items-center">
                                    ...
                                    <Infotip body={`Highest performance call in ${readableTimeInterval(activeIntervalUserMainStat)}`} />
                                </div>
                                <div className="flex gap-[5px]">
                                    <div className="rounded-[4px] w-4 h-4 flex items-center justify-center bg-[#21CB6B52] shrink-0">
                                        <Image 
                            ...
                            <div className="flex flex-col items-center gap-1 py-4 onest">
                                <div className="flex gap-1 items-center">
                                    ...
                                    <Infotip body={`Percentage of calls that reached 2x gains or more in ${readableTimeInterval(activeIntervalUserMainStat)}`} />
                                </div>
                                <div className="flex gap-[5px]">
                                    <div className="rounded-[4px] w-4 h-4 flex items-center justify-center bg-[#21CB6B52] shrink-0">
                                        <Image 
                                    ...
                                    <p className="font-bold text-base leading-5">{shownMainStatData?.winRate?.toFixed(2) || 0}%</p>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col items-center gap-1 py-4 onest">
                                <div className="flex gap-1 items-center">
                                    ...
                                    <Infotip body={`Average gains from all calls in ${readableTimeInterval(activeIntervalUserMainStat)}`} />
                                </div>
                                <div className="flex gap-[5px]">
                                    <div className="rounded-[4px] w-4 h-4 flex items-center justify-center bg-[#21CB6B52] shrink-0">
                                        <Image 
                                    ...
                                    <p className="font-bold text-base leading-5">{formatNumberNoLoop(shownMainStatData?.averagePerformance)}x</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="xl:w-[37%] w-full xl:pr-6 px-6 flex justify-end items-center gap-4">
                        ...
                        <TimeIntervalsFilterTabs
                            ...
                            selectedInterval={activeIntervalUserMainStat}
                            ...
                            <div className="flex gap-5">
                                ...
                                <div className="w-full border-l border-b relative border-[#494949]">
                                        {performanceRanges.slice().reverse().map((performanceRow, index) => {
                                            const value = graphDataToShow?.performanceCounts?.[performanceRow?.key] || 0;
                                            ...
                                            {allCallsData.map((call, index) => (
                                                <div key={index} className="grid grid-cols-[0.25fr_1.25fr_1fr_1.5fr_0.5fr_0.75fr_0.75fr_0.75fr] w-full justify-between onest text-xs leading-4 py-2 border-b border-[#404040]">
                                                    ...
                                                    <div className="flex items-center gap-2"> 
                                                        ...
                                                        onClick={() => router.push(`/alpha-profile/${call?.channelId || call?.groupId}`)}
                                                        className="flex hover:underline hover:cursor-pointer"
                                                    >
                                                        {call?.channelName || call?.groupName}
                                                    </button>
