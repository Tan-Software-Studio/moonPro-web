...
const Page = () => {
  ...
  const dataa = () => {
    const startIndex = entriesPerPages * (currentPage - 1);

    const endIndex = startIndex + entriesPerPages;

    const data1 = data.slice(startIndex, endIndex);


    totalPages = Math.ceil(data.length / entriesPerPages);

    pages(totalPages);
  };

  const pages = () => {
    const start = currentPage === 1 ? 1 : currentPage - 1;

    const end = currentPage < totalPages ? currentPage + 1 : totalPages;

    const data3 = [start, currentPage, end];
    setData2(data3);
  };
  ...
  return (
    <>
      ...
      <div onClick={dataa} className="cursor-pointer">
        hiiiiiiiiiiiii
      </div>
      ...
      <div className="flex items-center justify-center gap-5 cursor-pointer">
        ...
        {data2.map((data, index) => (
          <>
            <div
              key={index}
              className={` rounded-full px-3 text-xl ${
                data === currentPage
                  ? "bg-gray-200 ease-in-out transition-all  "
                  : "border-[#171715]  border-[2px]"
              }`}
