...
export default function Portfolio() {
  ...
  const getAllWallets = async (e) => {
    ...
    try {
      ...
      console.log("🚀 ~ getAllWallets ~ response:", response?.data?.data?.wallets?.walletAddressSOL)
  ...
  async function handlePrimary(index) {
    ...
    if (!jwtToken) return 0;
    try {
      await axios.put(`${baseUrl}user/makeSolWalletPrimary`,
        {
          index
        },
        {
    ...
    <div className="bg-[#08080E] text-white p-4 md:p-6">
      <div className="border-[#404040] border-[1px] rounded-lg">
        <div className="flex flex-col  ">
          {/* Header & Search Bar */}
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between border-[#404040] border-b-[1px] 
            gap-4 px-3 py-3">
            ...
            <table className="w-full table-fixed overflow-x-auto">
              <thead>
                <tr className="text-left text-gray-400 text-sm border-[#404040] border-b-[1px]">
                  ...
                  <th className="py-3 px-5 font-normal w-2/12 text-center md:text-left">Balance</th>
                  <th className="py-3 px-5 font-normal w-2/12 text-center md:text-left">Holdings</th>
                  <th className="py-3 px-5 font-normal w-3/12 text-right">Actions</th>
                </tr>
              </thead>
              <tbody className=' ' >
                {walletAddresses.map((wallet, index) => (
                  <tr key={index}
                    ...
                    <td className="py-2 px-6 flex items-center gap-3 ">
                      <div className={`font-medium ${wallet.primary ? 'text-[rgb(247,147,26)]' : ""}`}>Wallet</div>
